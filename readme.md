...
- Can't it be true, EZ
- Cipher : /2&//<|0, EZ
- Red Rave, all three difficulty levels
...
# Installation
These are [MelonLoader](https://melonwiki.xyz/) mods. In order to run these, you need to have it installed. Currently, MelonLoader 0.7.3 is supported.  
...
_tl;dr: update to (supported by this repo) MelonLoader 0.7.x, or `tail -f MelonLoader/Latest.log`_

For whatever reason, the built-in MelonLoader 0.6.x's console does not display logs after the game (as opposed to the config app) starts. This includes messages about loading the mods and any messages (warnings, errors) they produce.
